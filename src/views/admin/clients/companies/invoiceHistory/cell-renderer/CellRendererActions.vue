<template>
  <div class="flex items-center h-100" :style="{'direction': $vs.rtl ? 'rtl' : 'ltr'}">
    <edit-icon
      ...
      @click="$router.push({ name: 'admin-vendors-did-edit', params: { did: params.data.did } })"
    />
    <trash-2-icon
      size="1.3x"
...
export default {
  ...
  methods: {
    ...
    Delete () {
      this.$store.dispatch("did/Delete", this.params.data.did)
      .then(() => {
        this.params.data.refreshData()
        this.ShowSuccessMessage({
          title: 'DID Deleted',
          text: 'The selected DID was successfully deleted'
        })
      })
